...
export default async function AssistantEditorWithId({ params }: PageProps) {
  const { assistantId: routeAssistantId } = await params;
  ...
  if (userId && email) {
    ...
    const fromRoute = assistants.find((a) => a.id === routeAssistantId);
    if (fromRoute) {
      assistantId = fromRoute.id;
