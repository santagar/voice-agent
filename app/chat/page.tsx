...
export default function MobileVoiceChat() {
  ...
  return (
    <main
      className="h-dvh bg-slate-950 text-slate-100 overflow-hidden"
      style={{ backgroundImage: labTheme.gradients.canvas }}
    >
      <div className="mx-auto flex h-full max-w-3xl flex-col gap-4 p-4 pb-4 sm:pb-6">
        <Card
          className="border-white/10 bg-white/5 p-0 shadow-lg flex flex-1 flex-col min-h-0"
          padding="p-0"
        >
          <div className="flex items-start justify-between px-4 py-3">
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-2 text-[11px] text-slate-300">
                <span
                  className={`status-dot ${wsConnected ? "dot-online" : "dot-offline"}`}
                />
            ...
            <div className="flex items-center gap-2">
              <div className="hidden items-center gap-2 sm:flex">
                ...
                >
                  {muted ? (
                    <VolumeX strokeWidth={1.8} className="h-4 w-4" />
                  ) : (
                    <Volume2 strokeWidth={1.8} className="h-4 w-4" />
                  )}
                </button>
                <button
                  onClick={toggleMic}
                ...
                >
                  {micMuted ? (
                    <MicOff strokeWidth={1.8} className="h-4 w-4" />
                  ) : (
                    <Mic strokeWidth={1.8} className="h-4 w-4" />
                  )}
                </button>
              </div>
              <button
                onClick={() => setShowMobileControls((prev) => !prev)}
                className={`rounded-full border p-2 transition sm:hidden ${
                  showMobileControls
          ...
          <div
            ref={chatRef}
            className="relative flex-1 min-h-0 overflow-y-auto bg-slate-950/60 px-4 py-4 pb-4 sm:pb-8"
          >
                ...
                return isAssistant ? (
                  ...
                  <ChatBubble
                    ...
                    className={`rounded-[18px] ${
                      message.from === "user" ? "text-base leading-relaxed" : ""
                    }`}
                  >
                    {message.text}
                  </ChatBubble>
          ...
          <div className="border-t border-white/5 px-4 py-3">
            <div className="flex items-center gap-3">
              <div className="relative flex-1">
                <input
                  type="text"
                  className="h-14 w-full rounded-full border border-white/10 bg-white/5 pl-5 pr-16 text-base text-white placeholder:text-slate-500 focus:outline-none"
                  placeholder={
                    callStatus === "in_call"
        ...
        {showMobileControls && (
          <div className="sm:hidden">
            <div className="mx-auto mt-3 flex w-full max-w-md items-center justify-center gap-4 px-4">
              <button
                onClick={toggleSpeaker}
              ...
              <button
                ...
                className={`flex h-16 w-16 items-center justify-center rounded-full border text-white shadow-lg transition active:scale-95 ${
                  callStatus === "in_call"
                    ? "border-rose-500/50 bg-rose-500 shadow-rose-900/40"
                    : callStatus === "calling"
                    ? "border-amber-400/60 bg-amber-500 shadow-amber-900/40 animate-pulse"
                    : "border-emerald-400/60 bg-emerald-500 shadow-emerald-900/40"
                }`}
